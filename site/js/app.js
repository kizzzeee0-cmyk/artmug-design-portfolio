...
function visibleCats(xs){return (xs||[]).filter(x=>x.enabled!==false)}function renderDesignTypes(types){const shown=visibleCats(types);$('designTypeInput').innerHTML=shown.map(x=>`<option value="${esc(x.id)}">${esc(x.label)}</option>`).join('');updateDesignType()}
function currentType(){return (S?.designTypes||[]).find(x=>x.id===$('designTypeInput').value)||{} }function updateDesignType(){const t=currentType();$('frameKeepField').hidden=!t.showFrameRetention;$('signatureFields').hidden=!t.showSignatureFields;$('bannerFields').hidden=!t.showBannerFields;$('reviewEventField').hidden=!t.showReviewEvent}
